...
signal hit_by_damage(damage: DamageData, source: Entity)
## Triggered whenever an effect is applied to the hitbox.
signal hit_by_effect(effect: EffectData, source: Entity)
## Triggered whenever the hitbox is knocked back by an attack.
signal hit_by_knockback(knockback: KnockbackData, source: Entity)
...
func handle_collision(collision: CollisionData, source: Entity) -> void:
	if detect_only:
		return
...
func handle_damage(damage: DamageData, source: Entity) -> void:
	if detect_only:
		return
	hit_by_damage.emit(damage, source)


func handle_effect(effect: EffectData, source: Entity) -> void:
	if detect_only:
		return
	hit_by_effect.emit(effect, source)


func handle_knockback(knockback: KnockbackData, source: Entity) -> void:
	if detect_only:
		return
