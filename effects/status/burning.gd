...
## Deals fire damage at regular intervals while active. Damage scales with number of stacks.
...
func process(delta: float) -> void:
	if !health_component:
		...
		if interval_timer >= interval_time:
			health_component.damage(damage_amount * stacks, Enums.DamageType.Fire)
